import { pgTable, index, pgPolicy, uuid, varchar, text, timestamp, unique, foreignKey, check, numeric, integer, jsonb, boolean, date, primaryKey } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

// Note: 'users' table is likely a legacy table. 
// We should primarily use 'profiles' which references 'auth.users'.
// However, existing foreign keys in 'bookings', 'reviews', 'blog_posts' point to 'users'.
// Ideally, we should migrate these FKs to point to 'profiles' or 'auth.users', 
// but to respect "don't create new stuff", we keep the schema as introspected.
...
export const users = pgTable("users", {
    ...
    foreignKey({
        columns: [table.id],
        foreignColumns: [table.id], // Self-referencing FK? This looks suspicious in the DB but we keep it as introspected.
        name: "users_id_fkey"
    }).onDelete("cascade"),
]);
...
export const profiles = pgTable("profiles", {
    ...
    // The introspection shows profiles.id references users.id. 
    // This confirms 'users' table is acting as a bridge or legacy user table.
    foreignKey({
        columns: [table.id],
